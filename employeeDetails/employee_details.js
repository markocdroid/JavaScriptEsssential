...
function findEmployeeBySpecialization(specialization) {
    ...
    const specEmployeesDisplay = specializedEmployees.map((employee, index) => `<p>${employee.id}: ${employee.name}: ${employee.name} - ${employee.specialization} - $${employee.salary}</p>`).join('');
